...
function Floe(poly::LG.Polygon, hmean, Δh; ρi = 920.0, u = 0.0, v = 0.0, ξ = 0.0, mc_n = 1000.0, t::Type{T} = Float64) where T
    ...
    height = hmean + (-1)^rand(0:1) * rand() * Δh
...
"""
Floe(coords::PolyVec, h_mean, Δh; ρi = 920.0, u = 0.0, v = 0.0, ξ = 0.0, mc_n = 1000, t::Type{T} = Float64) where T =
...
Inputs:
        ...
        min_floe_area   <Float> minimum area for floe creation
Output:
...
"""
function poly_to_floes(floe_poly, h_mean, Δh, ρi, mc_n, min_floe_area = 0, t::Type{T} = Float64) where T
...
"""
function initialize_floe_field(coords::Vector{PolyVec{T}}, domain, h_mean, Δh; min_floe_area = 0.0, ρi = 920.0, mc_n::Int = 1000, t::Type{T} = Float64) where T
    ...
    for p in floe_polys
        append!(floe_arr, poly_to_floes(p, h_mean, Δh, ρi, mc_n, min_floe_area, T))
    end
...
"""
function initialize_floe_field(nfloes::Int, concentrations, domain, h_mean, Δh; min_floe_area = 0.0, ρi = 920.0, mc_n::Int = 1000, t::Type{T} = Float64) where T
    floe_arr = StructArray{Floe{T}}(undef, 0)
        ...
        for j in range(1, ncols)
            ...
            if c > 0
                ...
                while !isempty(floe_idx) && floes_area/open_area <= c
                    ...
                    floes = poly_to_floes(floe_poly, h_mean, Δh, ρi, mc_n, min_floe_area, T)
