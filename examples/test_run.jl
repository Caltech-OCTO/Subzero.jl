...
# Floe instantiation
floe_arr = initialize_floe_field(100, [0.7], domain, hmean, Δh, rng = Xoshiro(1), nhistory = 1000)
...
initwriter = InitialStateOutputWriter(dir = "output/sim", filename = string(tstring, "_initial_state.jld2"), overwrite = true)
gridwriter = GridOutputWriter(100, simulation.model.grid, (10, 10), dir = "output/sim", filename = string(tstring, "_grid.nc"), overwrite = true)
floewriter = FloeOutputWriter(100, dir = "output/sim", filename = string(tstring, "_floes.jld2"), overwrite = true)
checkpointwriter = CheckpointOutputWriter(500, dir = "output/sim", filename = string(tstring, "_checkpointer.jld2"), overwrite = true)
